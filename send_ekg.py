...
DEFAULT_PORT = "COM5"        # promijeni po potrebi
DEFAULT_BAUD = 115200
DEFAULT_BLOCK = 32  # mora odgovarati STM32
...
def choose_format(samples, force_signed=False, force_unsigned=False):
    ...
    # auto-detect: if there are negative values -> signed
    if mn < 0:
        if mn < -32768 or mx > 32767:
            raise ValueError("Values out of int16 range for signed format")
        return "<h"
